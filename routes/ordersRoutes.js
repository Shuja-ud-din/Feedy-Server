...
ordersRoutes.post("/createOrder", authentication, async (req, res) => {
    const { userId, products, orderLocation, shippingDetails, paymentDetails = {
        shippingPrice: 0,
        taxPrice: 0
    } } = req.body;
    ...
    let orderedProducts = await Product.find({ id: { $in: products.map(item => item.productId) } });
    ...
    try {
        const orderDetails = await Order.create({
            ...
            discount,
            totalPrice: totalPrice - discount + paymentDetails.taxPrice + paymentDetails.shippingPrice,
        });
